...
public class VirtualPlayer : MonoBehaviour
{
    ...
    protected virtual void PlanInit(int stage)
    {
        ...
        if( stage == 1 )
        {
            // 함수 사용법: planList.Add(new Plan(움직일 기물의 좌표(x값), 움직일 기물의 좌표(y값), 기물을 움직일 좌표(x값), 기물을 움직일 좌표(y값));
            // 예시: planList.Add(new Plan(1, 1, 2, 2)); <<< AI가 1,1 좌표에 있던 기물을 2,2로 옮긴다.
            // 먼저 진행할 순서대로 AI의 수를 입력
        }
    }
